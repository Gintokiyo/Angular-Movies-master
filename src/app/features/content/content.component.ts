...
export class ContentComponent implements OnInit {
  ...
  getMoviesExternal(name: string, page: number | undefined): void {
    console.log(name);
  ...
  getMoviesDatabase(name: string): void {
    console.log(name);
    this.searchKeyword = name;
    this.searchExists = name === "" ? false : true;
    ...
    if (this.searchExists) {
      ...
      .subscribe((res) => {
        ...
        movieList = this.mapDatabaseMovies(res).slice(0, 19);
        ...
        this.paginationObject.total_pages = Math.ceil(
          this.paginationObject.total_results / 20
        );
  ...
  getSeries(page: number): void {
    ...
    this.databaseService
      ...
      .subscribe((res) => {
        let movieList = new Array<MovieModel>();
        movieList = this.mapDatabaseMovies(res);
  ...
  changePage(event) {
    if (this.contentType === "movies") {
      if (this.searchExists) {
        this.getMoviesExternal(this.searchKeyword, event.pageIndex + 1);
      } else {
  ...
  mapDatabaseMovies(intake: any): Array<MovieModel> {
    let movieList = new Array<MovieModel>();
    intake.forEach((element) => {
      let movie: MovieModel = {
        ...
        release_date: new Date(element.seriesReleaseDate).toDateString(),
        overview: element.seriesDescription,
        homepage: element.seriesHomepage,
        adult: false,
        genres: undefined,
        imdb_id: undefined,
        popularity: element.seriesExternalId,
